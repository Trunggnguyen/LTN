...
object CommonUtils {
    ...
    private val KEYBOARD_VISIBLE_THRESHOLD_DP = 100f
    ...
    fun setStatusBarColorBlack(activity: Activity) {
...
// finally change the color
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.statusBarColor = ContextCompat.getColor(activity, R.color.colorBG)
        }
    }
...
// finally change the color
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.statusBarColor = ContextCompat.getColor(activity, R.color.ef_white)
        }
    }
